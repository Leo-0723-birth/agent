...
def test_invalid_as_of_is_422():
    with TestClient(main.app) as client:
        response = client.post("/api/scan", json={"code": "000001.SZ", "as_of": "not-a-date"})
    assert response.status_code == 422


def test_as_of_selects_latest_report_not_after_requested_date(tmp_path, monkeypatch):
    old = _report("000001.SZ", "2026-08-20", 0.2)
    new = _report("000001.SZ", "2026-08-28", 0.8)
    ...
    manifest = [
        {"company": "000001.SZ", "as_of": "2026-08-20", "generated_at": old["generated_at"], "json_file": "old.json"},
        {"company": "000001.SZ", "as_of": "2026-08-28", "generated_at": new["generated_at"], "json_file": "new.json"},
    ]
    ...
    selected = pipeline._load_report("000001.SZ", "2026-08-25")
...
def test_unpublishable_runtime_falls_back_instead_of_completed(monkeypatch):
    ...
    asyncio.run(pipeline.run_scan_task(state, ScanRequest(code="000001.SZ", realtime=True)))
...
def test_legacy_report_is_explicitly_unverified():
    response = pipeline.offline_to_response_from_report(_report("000001.SZ", "2026-08-20", 0.2))
