...
# ---------- 代码格式与行情快照工具（供 market_fetch / f2_calc 使用） ----------
...
def _get_quote_snapshot(company_code):
    """腾讯行情接口：返回当前市值/PE/PB 快照（市值单位元）。失败返回 {}。"""
    market = _market_of(company_code).lower()
    code6 = _to_6digit(company_code)
    try:
        r = requests.get(f"https://qt.gtimg.cn/q={market}{code6}", timeout=10)
        r.encoding = "gbk"
        if '="' not in r.text:
            return {}
        f = r.text.split('="', 1)[1].rstrip('";\n').split("~")
        return {
            "pe_ratio": float(f[39]) if f[39] else None,          # [39]=市盈率
            "pb_ratio": float(f[46]) if f[46] else None,          # [46]=市净率
            "market_cap": float(f[45]) * 1e8 if f[45] else None,  # [45]=总市值(亿) -> 元
        }
    except Exception as e:
        print(f"  [行情快照失败] {company_code}: {e}")
        return {}
...
class DataFetcher:
    ...
    def fetch_financials(self, stock_code, page_size=20):
        ...
        df_out["company_code"] = secucode                    # F2 计算需要的字段
        ...
        # 行业（全期相同；失败为 None）
        profile = self.fetch_company_profile(secucode)
        df_out["industry"] = (profile or {}).get("industry")
        # 行情快照（市值/PE/PB，只填最新一期；其余行 NaN 并转数值类型）
        quote = _get_quote_snapshot(secucode)
        for col in ("market_cap", "pe_ratio", "pb_ratio"):
            df_out[col] = float("nan")
            if quote and len(df_out) > 0:
                df_out.loc[df_out.index[-1], col] = quote.get(col)
