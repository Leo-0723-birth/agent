...
class AnnouncementReaderAgent(AgentBase):
    ...
    def __init__(self, data_root=None, use_finbert=True, use_llm=True,
...
用户输入：{user_input}"""
    ...
    def llm_extract(self, company_name, announcements):
        print("[DEBUG] llm_extract called:", len(announcements))
...
- 只输出 JSON 对象本身。"""
    ...
    def execute(self, company, ctx):
        ...
        # 4.5 被FinBERT过滤的公告补标记
        filtered_ids = {a["id"] for a in announcements} - {a["id"] for a in llm_candidates}
