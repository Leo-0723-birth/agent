...
class SweepingOrchestrator(AgentBase):
    ...
    def __init__(self, use_llm=True, use_finbert=True, rate_limit=0.5):
        super().__init__()
    ...
    def execute(self, company, ctx):
        ...
        self._run_risk_mapper(company, ctx)    # Phase 2
        self._run_financial(company, ctx)      # Phase 3
        self._run_predict(company, ctx)        # Phase 4（占位，待填充）
        self._run_cases(company, ctx)          # Phase 5
        self._run_attribution(company, ctx)    # Phase 6
        return ctx

    def _run_risk_mapper(self, company, ctx):

        from .risk_mapper import RiskMapperAgent

        agent = RiskMapperAgent()
